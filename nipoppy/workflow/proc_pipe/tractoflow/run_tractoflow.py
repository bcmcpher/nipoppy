...
def max_lmax(ndirs, symmetric=True):
    ...
    # return the (fixed) harmonic order
    return (lmax)
...
class BIDSdwi():
    ...
    def __init__(self, dwi, layout, b0thr=45, stol=50):
        ...
        if self.bval.shape[0] != self.nvol:
            ...
            print("All data (nii.gz, bval, bvec) have matching dimensions.")
...
# get the diffusion phase encoding of all inputs in a list
...
for axis in dAxes:
    ...
    # pull the elements from the list
    ipe = axis[0]
    ipe_dir = axis[1]
    ipe_b0s = axis[2]
    idfiles = axis[3]

    # get the n of volume types
    n_ipe_dir = np.sum(ipe_dir)
    n_ipe_b0s = np.sum(ipe_b0s)

    print(f"Phase Encoding {ipe} has {len(idfiles)} input(s).")
...
#        - CHECK THAT BVECS ARE NOT DUPLICATES
#        - DEAL WITH B0 VOLUMES W/ SOME DIRECTION
#        - OPTIONS TO ADD EXTRA B0S

# ONCE THE INPUTS ARE PARSED AND OPTIONALLY MERGED, CHECK PE DIRS FOR FPE / RPE PAIRS



# # # below here is junk / old logic until:  def parse_data():
...
if ndvols == 1:
    ...
    if dphaseAxis[0] == dphaseAxis[1]:
        ...
        print("Different encoding axes. Weird.")
